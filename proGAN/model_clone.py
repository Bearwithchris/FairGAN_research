...
def build_256x256_generator(noise_dim=NOISE_DIM):
    ...
    x, _ = mt.upsample_block(x, in_filters=128, filters=64, kernel_size=3, strides=1,
                                         padding='same', activation=tf.nn.leaky_relu, name='Up_{}x{}'.format(64, 64))
    x, _ = mt.upsample_block(x, in_filters=64, filters=32, kernel_size=3, strides=1,
                                         padding='same', activation=tf.nn.leaky_relu, name='Up_{}x{}'.format(128, 128))
    ########################
    # Fade in block
    ########################
    x, up_x = mt.upsample_block(x, in_filters=32, filters=16, kernel_size=3, strides=1,
...
def build_512x512_generator(noise_dim=NOISE_DIM):
    ...
    x, _ = mt.upsample_block(x, in_filters=64, filters=32, kernel_size=3, strides=1,
                                         padding='same', activation=tf.nn.leaky_relu, name='Up_{}x{}'.format(128, 128))
    x, _ = mt.upsample_block(x, in_filters=32, filters=16, kernel_size=3, strides=1,
                                         padding='same', activation=tf.nn.leaky_relu, name='Up_{}x{}'.format(256, 256))
    ########################
    # Fade in block
    ########################
    x, up_x = mt.upsample_block(x, in_filters=16, filters=8, kernel_size=3, strides=1,
...
def build_256x256_discriminator():
    ...
    # Left branch in the paper
    ########################
    previous_from_rgb = mt. EqualizeLearningRate(tf.keras.layers.Conv2D(32, kernel_size=1, strides=1, padding='same', activation=tf.nn.leaky_relu,
                      kernel_initializer=kernel_initializer, bias_initializer='zeros'), name='from_rgb_{}x{}'.format(128, 128))
    l_x = previous_from_rgb(downsample(inputs))
    l_x = tf.keras.layers.Multiply()([1 - alpha, l_x])
    ########################
    # Right branch in the paper
    ########################
    from_rgb = mt. EqualizeLearningRate(tf.keras.layers.Conv2D(16, kernel_size=1, strides=1, padding='same', activation=tf.nn.leaky_relu,
                      kernel_initializer=kernel_initializer, bias_initializer='zeros'), name='from_rgb_{}x{}'.format(256, 256))
    r_x = from_rgb(inputs)
    ########################
    # Fade in block
    ########################
    r_x = mt.downsample_block(r_x, filters1=16, filters2=fade_in_channel, kernel_size=3, strides=1,
    ...
    # Stable blocks
    ########################
    x = mt.downsample_block(x, filters1=32, filters2=64, kernel_size=3, strides=1,
                                            padding='same', activation=tf.nn.leaky_relu, name='Down_{}x{}'.format(128,128))
    x = mt.downsample_block(x, filters1=64, filters2=128, kernel_size=3, strides=1,
                                            padding='same', activation=tf.nn.leaky_relu, name='Down_{}x{}'.format(64,64))
    x = mt.downsample_block(x, filters1=128, filters2=256, kernel_size=3, strides=1,
...
def build_512x512_discriminator():
    ...
    # Left branch in the paper
    ########################
    previous_from_rgb = mt. EqualizeLearningRate(tf.keras.layers.Conv2D(16, kernel_size=1, strides=1, padding='same', activation=tf.nn.leaky_relu,
                      kernel_initializer=kernel_initializer, bias_initializer='zeros'), name='from_rgb_{}x{}'.format(256, 256))
    l_x = previous_from_rgb(downsample(inputs))
    l_x = tf.keras.layers.Multiply()([1 - alpha, l_x])
    ########################
    # Right branch in the paper
    ########################
    from_rgb = mt. EqualizeLearningRate(tf.keras.layers.Conv2D(8, kernel_size=1, strides=1, padding='same', activation=tf.nn.leaky_relu,
                      kernel_initializer=kernel_initializer, bias_initializer='zeros'), name='from_rgb_{}x{}'.format(512, 512))
    r_x = from_rgb(inputs)
    ########################
    # Fade in block
    ########################
    r_x = mt.downsample_block(r_x, filters1=8, filters2=fade_in_channel, kernel_size=3, strides=1,
    ...
    # Stable blocks
    ########################
    x = mt.downsample_block(x, filters1=8, filters2=32, kernel_size=3, strides=1,
                                            padding='same', activation=tf.nn.leaky_relu, name='Down_{}x{}'.format(256,256))
    x = mt.downsample_block(x, filters1=32, filters2=64, kernel_size=3, strides=1,
                                            padding='same', activation=tf.nn.leaky_relu, name='Down_{}x{}'.format(128,128))
    x = mt.downsample_block(x, filters1=64, filters2=128, kernel_size=3, strides=1,
