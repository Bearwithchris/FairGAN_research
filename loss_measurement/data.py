...
dir1 = 'D:/GIT/local_data_in_use/Train/Male'
dir2 = 'D:/GIT/local_data_in_use/Train/female'

#Test Data Prep


def loadImgs_Test(dir_images,size):
    cap=20
    count=0
    imagelist=[]
...
#Training Data Prep
# def loadImgs(size,dir_images,cap=5000):
#     # cap=5000
#     count=0
#     imagelist=[]
#     images_paths=os.listdir(dir_images)
...
def loadImgs_random(dir_images,size,cap=1000):
    # cap=5000
    # imagelist=np.array([])
    # imagelist=[]
    images_paths=os.listdir(dir_images)
...
def prep_dataset(dim,directory_male,directory_female,sample):
    sample_male=sample/2
    sample_female=sample-sample_male
    
    size=[dim,dim]
    images_male=loadImgs_random(directory_male,size,sample_male)
    images_male_labels=np.ones(len(images_male))
    images_female=loadImgs_random(directory_female,size,sample_female)
    images_female_labels=np.zeros(len(images_female))
    images_concat=np.concatenate((images_male,images_female),axis=0)
    images_labels_concat=np.concatenate((images_male_labels,images_female_labels))
    images_concat_shuffled,images_labels_concat_shuffled=shuffle(images_concat,images_labels_concat)
...
#============================================================================================================================================
def prep_bias_data(directory_male,directory_female,samples,size,bias=0.5):
    female_samples=samples*bias
    male_samples=samples*(1.0-bias)
    
    #Load_bias_data
    images_male=loadImgs_random(directory_male,size,male_samples)
    images_female=loadImgs_random(directory_female,size,female_samples)
    images_concat=np.concatenate((images_male,images_female),axis=0)
...
def datasets(dim,dir1,dir2,samples,bias,gamma=1.0,evaluation=True):
    size=[dim,dim]
    bias_data_samples=samples/(gamma+1)
    ref_data_samples=samples-bias_data_samples
    
    bias=prep_bias_data(dir1,dir2,bias_data_samples,size,bias)
    bias_labels=np.ones(len(bias)) # Bias data
    
    ref=prep_bias_data(dir1,dir2,ref_data_samples,size)
    ref_labels=np.zeros(len(ref)) # Unbias data
...
# labels,images=datasets(128,dir1,dir2,13000,bias=0.9)
